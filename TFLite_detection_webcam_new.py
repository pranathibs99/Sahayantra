...
pb = Pushbullet("o.MyUi0AsThIUR2L1dP95PU0nFM6nKMYlq")
...
mic_name1="USB PnP Sound Device: Audio (hw:1,0)"
mic_name2="USB PnP Sound Device: Audio (hw:0,0)"
sample_rate=44100
chunk_size=512
label_out=[]
mid_x_out=[]
...
def speech_recognize():
    ...
    for i,microphone_name in enumerate(mic_list):
        ...
        if microphone_name==mic_name:
            device_id=i
        if microphone_name==mic_name1:
            device_id=i
        if microphone_name==mic_name2:
            device_id=i
...
def scan(speech_input):
    ...
    for i in label1:
        if speech_input in i or 'toilet' in i:
...
while(True):
    now = datetime.now()
    morning_8am = now.replace(hour = 8, minute =0,second =0, microsecond=0)
    morning_8_30am = now.replace(hour = 8,minute= 30,second = 0,microsecond = 0)
    if now>morning_8am and now<morning_8_30am:
        if flag == 0:
            print("yes")
            main("cup")
            flag=1
    afternoon_1_30pm = now.replace(hour = 13, minute =30,second =0, microsecond=0)
    afternoon_2pm = now.replace(hour = 14,minute= 0,second = 0,microsecond = 0)
    if now>afternoon_1_30pm and now<afternoon_2pm:
        if flag1 == 0:
            print("yes")
            main("cup")
            flag1=1
    night_8pm = now.replace(hour = 0, minute =0,second =0, microsecond=0)
    night_8_30pm = now.replace(hour = 0,minute= 30,second = 0,microsecond = 0)
    if now>night_8pm and now<night_8_30pm:
        if flag2 == 0:
            print("yes")
            main("cup")
            flag2=1
    ...
    if txt == "help":
        dev = pb.get_device('OnePlus 6')
        push = dev.push_note("Alert!!", "Patient needs help immidiately")
    main("cup")